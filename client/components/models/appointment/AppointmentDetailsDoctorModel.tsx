      ...
      { appointmentId, status: AppointmentStatus.CONFIRMED },
      {
        onSuccess: () => {
          toast({
            ...
            description: "✅ Your appointment has been successfully confirmed! The patient will be notified shortly.",
            variant: "success",
          });
          refetch();
        ...
        onError: (error) => {
          toast({
            ...
            description: error?.response?.data?.message || "⚠️ Oops! Something went wrong. Please try again later.",
            variant: "destructive",
          });
        },
      }
        ...
        <AlertDialogFooter className="flex justify-between">
          ...
          {appointment && (appointment.status === AppointmentStatus.PENDING || appointment.status === AppointmentStatus.CONFIRMED) && (
            <div className="space-x-2">
              <Button variant="destructive" onClick={showCancellationModal}>Reject</Button>
              {appointment.status === AppointmentStatus.PENDING && (
                <Button variant="success" onClick={handleAcceptAppointment}>Accept</Button>
              )}
            </div>
          )}

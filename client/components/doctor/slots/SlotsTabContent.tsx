...
const TabContent = ({ day }: Props) => {
   ...
   useEffect(() => {
      if (!isLoading && slots) {
         setSelectedMorningSlots(
            slots.filter((el) => AvailableTimes.Morning.includes(el.startTime || "")).map((el) => el.startTime || "")
         );
         setSelectedAfternoonSlots(
            slots.filter((el) => AvailableTimes.Afternoon.includes(el.startTime || "")).map((el) => el.startTime || "")
         );
         setSelectedEveningSlots(
            slots.filter((el) => AvailableTimes.Evening.includes(el.startTime || "")).map((el) => el.startTime || "")
         );
      }
   }, [slots, isLoading]);
   ...
   return (
      <div className="p-2 sm:p-4 bg-dark-200 rounded-lg shadow-md">
         <h1 className="text-lg sm:text-xl font-medium mb-4 text-white">
            Slots for <span className="font-semibold">{day.toUpperCase()}</span>
         </h1>

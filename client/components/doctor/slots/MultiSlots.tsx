...
const SlotManager = () => {
    ...
    const handleAddSlots = () => {
        if (slotsToAdd.length > 0) {
            addSlots({ startTimes: slotsToAdd }, {
                onSuccess: () => {
                    setSlotsToAdd([]);
                    toast({
                        title: "Slots Added",
    ...
    const handleDeleteSlots = () => {
        if (slotsToRemove.length > 0) {
            deleteSlots({ startTimes: slotsToRemove }, {
                onSuccess: () => {
                    setSlotsToRemove([]);
                    toast({
                        title: "Slots Removed",
    ...
    return (
        <div className="space-y-4">
            ...
            <ConfirmDeleteSlotsModel open={isDeleteModelOpen} handleDeleteSlots={handleDeleteSlots} setOpen={setDeleteModelOpen} />
        </div>
    );
}

    ...
    <div className="min-h-screen p-4 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        {data?.items.map((appointment) => (
          <Card key={appointment._id} className="mb-4 bg-dark-400 border-gray-400 shadow-md hover:shadow-lg transition-shadow duration-300">
            <CardHeader className="py-3 px-4 bg-gray-750 border-b border-gray-700">
              <CardTitle className="flex justify-between items-center text-base">
                <div className="flex items-center space-x-2">
                  <Calendar className="w-4 h-4 text-green-400" />
                  <span className="text-gray-100">{format(new Date(appointment.appointmentDate!), "dd/MM/yy")}</span>
                </div>
